...
#take apcli parameters as configs of vif(ra0)    // by noel
    config_get ApCliEnable "$vif" ApCliEnable
	...
	echo "[  88  **  88  **  88  **  88  **]starting cat > /tmp/RT2860.dat"
...
BssidNum=${num:-1}
SSID1=${ssid1:-WRTnode_SSID1}
SSID2=${ssid2:-WRTnode_SSID2}
SSID3=${ssid3:-WRTnode_SSID3}
SSID4=${ssid4:-WRTnode_SSID4}
SSID5=
SSID6=
SSID7=
SSID8=
...
ApCliWPAPSK=$ApCliPassWord
ApCliDefaultKeyID=0
ApCliKey1Type=0
ApCliKey1Str=$ApCliPassWord
ApCliKey2Type=0
ApCliKey2Str=
ApCliKey3Type=0
ApCliKey3Str=
...
#传参过来的第一个参数是Radio0
	...
	insmod rt2860v2_ap  #//by noel
	ifconfig ra0 up  #with ra0 up, apcli0 up automatically//by noel
	...
	for vif in $vifs; do
		...
		#[ "$mode" == "sta" ] && {
					...
					#actually apcli do not work with iwpriv so following code are useless  //noel
					#ifname="apcli0"
...
#		uci set network.$net_cfg.type=bridge
		uci set network.$net_cfg.ifname=$ifname
		uci commit network
		}
		else
		{
		...
		if [ -z "$net_cfg" ]; then
			...
			if [ -z `brctl show |grep $ifname` ];then
			brctl addif br-$net_cfg $ifname
			fi
		fi
		}
		fi;
...
config wifi-iface
	...
	option network	lan
	option mode     ap
	option ssid     WRTnode${i#0}$(cat /sys/class/net/ra${i}/address|awk -F ":" '{print $5""$6 }'| tr a-z A-Z)
